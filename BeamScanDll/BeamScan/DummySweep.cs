    ...
    internal class DummySweep
    {
        ...
        public void Read(ref double[,] scans, int startIndex, int length)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                ...
                scans[startIndex + i, 2] = Parameter.CalibFocus.F0;//StaticTool.CaculateFocus((uint)StaticTool.GetRadius((uint)point.X, (uint)point.Y)) + Parameter.PreHeat.FocusOffset;//this.m_DummyScan[2];
                scans[startIndex + i, 3] = this.m_DummyScan[3];
                scans[startIndex + i, 4] = this.m_DummyScan[4];
                scans[startIndex + i, 5] = 32000;//this.m_DummyScan[5];
                scans[startIndex + i, 6] = this.m_DummyScan[6];
                scans[startIndex + i, 7] = this.m_DummyScan[7];
